...
mlflow.set_experiment(experiment_name='MLFlow moana-fl-fedopt-t1')
'''
class FedLearner(Learner):
    ...
    def __init__(
        ...
        lambda_vgg=1e-2,
        path_logs='logs/',
        path_model='/projects/I20240003/alicia.oliveira/moana-fl-fedopt-t1/',
        ...
        save_epoch=10,
        path_training_time='/projects/I20240003/alicia.oliveira/moana-fl-fedopt-t1/training_time.txt',
    ...
    def train(self, shareable: Shareable, fl_ctx: FLContext, abort_signal: Signal) -> Shareable:
        ...
        self.local_train(fl_ctx, weights_list, abort_signal)
        ...
        #print('new_weights_flat', new_weights_flat)
        #print('len new_weights_flat', len(new_weights_flat))

        # Initialize model_diff to have the same structure as new_weights_flat
        model_diff = {}
        
        for layer_name in new_weights_flat:
            global_w = model_weights[layer_name]  # Assuming model_weights contains the global weights
            local_w = new_weights_flat[layer_name]
            
            # Ensure global_w and local_w are both arrays, not lists of arrays
            if isinstance(global_w, list):
                global_w = np.array(global_w)
            if isinstance(local_w, list):
                local_w = np.array(local_w)
            
            # Calculate the difference
            diff = np.subtract(global_w, local_w)
            
            # Store the result in model_diff
            model_diff[layer_name] = diff
        
        #print('model_diff', model_diff)
        #print('len (model_diff)', len(model_diff))
        
        #dxo = DXO(data_kind=DataKind.WEIGHTS, data=new_weights_flat)
        dxo = DXO(data_kind=DataKind.WEIGHT_DIFF, data=model_diff)
        
        dxo.set_meta_prop(MetaKey.NUM_STEPS_CURRENT_ROUND, self.train_size_client)
